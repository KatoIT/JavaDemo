...
public class SymbolTable<Key extends Comparable<Key>, Value> implements Iterable<Key> {
	 ...
	 *         table; {@code null} if the key is not in this symbol table
	 * @throws IllegalArgumentException if {@code key} is {@code null}
	 */
	public Value get(Key string) {
		if (string == null)
			throw new IllegalArgumentException("calls get() with null key");
		return st.get(string);
	}
	 ...
	 * <p>
	 * To iterate over all of the keys in the symbol table named {@code st}, use the
	...
	/**
	 * Returns all of the keys in this symbol table. To iterate over all of the keys
	 * in a symbol table named {@code st}, use the foreach notation:
	 * {@code for (Key key : st)}.
	 * <p>
